...
The classification of P2P-TV traffic can be performed by exploiting the signature described before through any supervised learning machine. In this paper, we resort to several methods of machine learning algorithms such as Multi-instance Support Vector Machine, CitationkNN, etc., which are famous for their discriminative power in both the learning machine community\cite{svm}, and have more recently peered in the Internet traffic classification literature as well\cite{flow}. Specifically, these algorithms(Multi-instance Support Vector Machine(MISVM), CitationkNN, Multi-instance Diversity Dense(MIDD), Multi-instance Expectation-Maximization version of Diverse Density(MIEMDD), Multi-instance Wrapper(MIWrapper), Multi-instance Boost(MIBoost)) are all implemented in Weka3.6.5\cite{weka} which are applied in this article.

\subsection{Workflow overview}
From a high-level perspective, the P2P-TV classification workflow consists of three phases: the \emph{training} phase, whose output is a model that can be used for the second phase, the \emph{classification} phase. The third phase is to \emph{validate} the classification results against a reference ground truth. We will explain the workflow in detail with the help of Fig.2.
...
\bibitem[7]{svm}
N. Cristianini, J. Shawe-Taylor, An Introduction to Support Vector Machines and Other Kernel-based Learning Methods, Cambridge University Press, New York, NY, 1999

\bibitem[8]{flow}
N. Williams, S. Zander, G. Armitage, A preliminary performance comparison of five machine learning algorithms for practical IP traffic flow classification, ACM Computer Communication Review 36 (5) (2006) 5¨C16.
